<?php
session_start();
if (!isset($_SESSION['role']) || $_SESSION['role'] !== 'etudiant') {//verifie si l'utilisateur n'est pas etudiant
...
$stmt->execute([$id]);
$etu = $stmt->fetch();//selectionne un etudiant
$stmt = $pdo->prepare("SELECT n.note, m.intitule, m.code, m.coefficient FROM notes n JOIN modules m ON n.id_module=m.id WHERE n.id_etudiant=?");//recherche les modules et les note
$stmt->execute([$id]);
$notes = $stmt->fetchAll();//selectionne tous les note de ce etudiant
$moy_num = 0;
$moy_den = 0;
foreach ($notes as $n) {
  $moy_num += $n['note'] * $n['coefficient'];//add total
  $moy_den += $n['coefficient'];//add coefficient total
}
$moy = $moy_den > 0 ? $moy_num / $moy_den : 0;//calcule moyenne si coef!=0
?>
<div class="container">
